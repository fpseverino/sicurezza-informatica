...
public class AES implements Cipher {
    ...
    private static final String[] MULTIPLY_2 = {
    ...
    private static final String[] MULTIPLY_3 = {
    ...
    private static final String[] MULTIPLY_9 = {
    ...
    private static final String[] MULTIPLY_11 = {
    ...
    private static final String[] MULTIPLY_13 = {
    ...
    private static final String[] MULTIPLY_14 = {
    ...
    public String[][] stringToState(String binaryString) {
        ...
        for (int i = 0; i < 4; i++)
            for (int j = 0; j < 4; j++)
                state[i][j] = text.substring(8 * i + 2 * j, 8 * i + 2 * j + 2);
        return state;
    }
    ...
    public String stateToString(String[][] state) {
        ...
        for (int i = 0; i < 4; i++)
            for (int j = 0; j < 4; j++)
                text.append(state[i][j]);
        return Encryption.hexToBinary(text.toString());
    }
    ...
    private String[] subWord(String[] word) {
        String[] newWord = new String[4];
        for (int i = 0; i < 4; i++)
            newWord[i] = S_BOX[Integer.parseInt(word[i].substring(0, 1), 16)][Integer.parseInt(word[i].substring(1, 2), 16)];
        return newWord;
    }

    private String[][][] generateSubkey(String key) throws IllegalArgumentException {
        int R; // Number of round keys
        try {
            R = numberOfRounds(key) + 1;
        } catch (IllegalArgumentException e) {
            throw e;
        }
        int N = key.length() / 32; // Number of 32-bit words in key
        String hexKey = Encryption.binaryToHex(key);
        String[][] W = new String[4 * R][4];
        for (int i = 0; i < N; i++)
            for (int j = 0; j < 4; j++)
                W[i][j] = hexKey.substring(8 * i + 2 * j, 8 * i + 2 * j + 2);
        for (int i = N; i < 4 * R; i++) {
            ...
            for (int j = 0; j < 4; j++)
                W[i][j] = Encryption.hexXOR(W[i - N][j], temp[j]);
        }
        // Words were shifted right by 1 byte, temporary fix
        for (int i = 0; i < 4 * R; i++) {
            String temp = W[i][3];
            for (int j = 3; j > 0; j--)
                W[i][j] = W[i][j - 1];
        ...
        String[][][] subkey = new String[R][4][4];
        for (int i = 0; i < R; i++)
            for (int j = 0; j < 4; j++)
                for (int k = 0; k < 4; k++)
                    subkey[i][j][k] = W[4 * i + j][k];
        return subkey;
    }
    ...
    private String[][] addRoundKey(String[][] state, String[][] key) {
        ...
        for (int i = 0; i < 4; i++)
            for (int j = 0; j < 4; j++)
                newState[i][j] = Encryption.hexXOR(state[i][j], key[i][j]);
        return newState;
    }
    ...
    private String[][] subBytes(String[][] state) {
        ...
        for (int i = 0; i < 4; i++)
            for (int j = 0; j < 4; j++)
                newState[i][j] = S_BOX[Integer.parseInt(state[i][j].substring(0, 1), 16)][Integer.parseInt(state[i][j].substring(1, 2), 16)];
        return newState;
    }
    ...
    private String[][] inverseSubBytes(String[][] state) {
        ...
        for (int i = 0; i < 4; i++)
            for (int j = 0; j < 4; j++)
                newState[i][j] = INVERSE_S_BOX[Integer.parseInt(state[i][j].substring(0, 1), 16)][Integer.parseInt(state[i][j].substring(1, 2), 16)];
        return newState;
    }
