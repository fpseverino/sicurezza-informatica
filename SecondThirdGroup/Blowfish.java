...
public class Blowfish implements Cipher {
    ...
    private static final String[] S_BOX_1 = {
    ...
    private static final String[] S_BOX_2 = {
    ...
    private static final String[] S_BOX_3 = {
    ...
    private String[] sBox0 = new String[256];
    private String[] sBox1 = new String[256];
    private String[] sBox2 = new String[256];
    private String[] sBox3 = new String[256];
    private String[] p = new String[18];

    public Blowfish(String key) throws IllegalArgumentException {
        if (key.length() < 32 || key.length() > 448)
            throw new IllegalArgumentException("Key length must be between 32 and 448 bits");
        if (key.length() % 32 != 0)
            throw new IllegalArgumentException("Key length must be a multiple of 32 bits");
        while (key.length() < 576)
            key += key;
        key = key.substring(0, 576);
        String hexKey = Encryption.binaryToHex(key);
        for (int i = 0; i < 18; i++)
            p[i] = Encryption.hexXOR(P[i], hexKey.substring(i * 8, i * 8 + 8));
        for (int i = 0; i < 256; i++) {
            sBox0[i] = S_BOX_0[i];
            sBox1[i] = S_BOX_1[i];
            sBox2[i] = S_BOX_2[i];
    ...
    private String F(String input) {
        String byteA = input.substring(0, 2);
        String byteB = input.substring(2, 4);
        String byteC = input.substring(4, 6);
        String byteD = input.substring(6, 8);
        int indexA = Integer.parseInt(byteA, 16);
        int indexB = Integer.parseInt(byteB, 16);
        int indexC = Integer.parseInt(byteC, 16);
        int indexD = Integer.parseInt(byteD, 16);
        String sBox0Value = sBox0[indexA];
        ...
        String hexResult = Long.toHexString(result);
    ...
    public String encrypt(String plainText, String key) {
        String hexPlainText = Encryption.binaryToHex(plainText);
    ...
    public String decrypt(String cipherText, String key) {
        String hexCipherText = Encryption.binaryToHex(cipherText);
